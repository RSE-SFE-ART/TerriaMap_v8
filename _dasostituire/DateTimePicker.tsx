...
class DateTimePicker extends React.Component<PropsType> {
  ...
  renderCenturyGrid(datesObject: ObjectifiedDates) {
    ...
    const { t } = this.props;
    const text = t("text.century");
    if (datesObject.dates && datesObject.dates.length >= 12) {
      return (
        <Grid>
          <GridHeading>{text}</GridHeading>
          {centuries.map(c => (
            <DateButton
              key={c}
              css={`
  ...
  renderList(items: Date[]) {
    if (isDefined(items)) {
      ...
      //end
      const { t } = this.props;
      const text_time = t("text.time");
      return (
        <Grid>
          <GridHeading>{text_time}</GridHeading>
          <GridBody>
            {items.map(item => (
              <DateButton
              ...
              > 
                {isDefined(this.props.dateFormat)
                  ? (this.props.dateFormat === "mmmm" && isItalian
                      ? formatMonthIt(item)
                      : dateFormat(item, this.props.dateFormat as string))
                  : formatDateTime(item)}
